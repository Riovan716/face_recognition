...
export default function AdminTambahMatakuliah() {
  ...
  useEffect(() => {
    const fetchDosenList = async () => {
      try {
        const res = await axios.get("http://localhost:5000/dosen", {
          withCredentials: true,
        });
        setDosenList(res.data);
      } catch (err) {
        setMsg("Gagal mengambil data dosen");
  ...
  const handleSubmit = async (e) => {
    ...
    if (!selectedDosenId) {
      setMsg("Silakan pilih dosen yang akan mengajar mata kuliah ini");
      setShowModalError(true);
      return;
    }
  ...
  const handleConfirmSubmit = async () => {
    try {
      await axios.post(
        "http://localhost:5000/matakuliah/admin",
        {
          matakuliah,
          kodematakuliah,
          semester,
          ...
          password: dosenPassword, // password matakuliah
        },
        { withCredentials: true }
      );
    ...
    <div className="w-full h-[calc(100vh-4rem)] overflow-y-auto px-4 sm:px-6 py-0 pb-16">
      ...
      <div className="bg-white shadow-xl rounded-xl p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="relative group">
            <label className="block text-sm font-medium text-gray-700 mb-1">Pilih Dosen</label>
            <User
              size={18}
              className="absolute top-[38px] left-3 text-gray-500 group-focus-within:text-blue-500 transition-colors"
            />
            <select
              ...
              required
            >
              <option value="">Pilih dosen yang akan mengajar</option>
              {dosenList.map((dosen) => (
                <option key={dosen.id} value={dosen.id}>
                  {dosen.nip} - {dosen.name}
                </option>
              ))}
            </select>
          </div>
          ...
          <div className="relative group">
            <label className="block text-sm font-medium text-gray-700 mb-1">Password Matakuliah</label>
            <Key
              size={18}
              className="absolute top-[38px] left-3 text-gray-500 group-focus-within:text-blue-500 transition-colors"
            />
            ...
            <p className="text-xs text-gray-500 mt-1">
              Masukkan password matakuliah yang akan digunakan untuk akses
            </p>
          </div>
        ...
        <div className="fixed inset-0 z-[9999] bg-black/20 backdrop-blur-sm flex justify-center items-center">
          <div className="bg-white rounded-xl p-6 w-full max-w-sm text-center shadow-xl border-t-4 border-red-500">
            ...
            <button
              ...
              className="px-6 py-2.5 !bg-red-500 text-white font-semibold rounded-lg hover:!bg-red-600"
            >
              Tutup
            </button>
          </div>
